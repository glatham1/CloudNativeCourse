...
func RunCLI() {
	...
	key = "aaa65484abadc8cea5c90587bdc42c87"
	if key == "" {
	...
	fmt.Printf("Weather Information for %s\nWeather Type: %s\nTemperature: %.1fº\nPressure: %d\nHumidity: %d\nWind Speed: %.2f\n", location, conditions.Summary, conditions.Temperature.Fahrenheit(), conditions.pressure, conditions.humidity, conditions.WindSpeed)
}
